...
<p align="center">
  <a href="README.md">English version of the document</a>
</p>
...
- В третия слой ще се изобразяват всички помещения в сградите спрямо посочения етаж/level. Този слой ще се насложи директно върху 2рия слой само при определен Zoom (При прекалено отдалечаване на картата помещенията се скриват). Изчертаването на помещенията в този слой се извършва аналогично на сградите от слой 2.

Всеки polygon от слой 2 и 3 ще предоставя интеракция. При еднократно натискане ще се показва прозорче с има, кратко описание и бутон за навигация до обекта. При Swipe up върху прозорчето то ще се плъзва нагоре и ще заеме по-голяма част от еграна, като ще предоставя повече информация за обекта примерно: какъв тип е помещението, работно време (ако е административно звено), учебен график (ако е лаборатория или лекционна зала).
<img src="src/mytu_test.jpg" alt="MyTU Test App" width="100%">

Данните за слой 2 и 3 ще се съхраняват и извличат от DB.

За навигиране в приложението от точка до точка ще се построи граф, в който се търси най-кратък път чрез имплементиране на алгоритъма на Дейкстра. За всяко ключово място се създава връх в графа (врата за зала/стая, завой, край на коридор, вход в сграда и др.). Върховете се свързват чрез ребра. За всеки връх и ребро са имплементирани 3 флага:
- Флаг за забрана за използване на върха/реброто при определяне на път изцяло;
- Флаг за забрана за използване на върха/реброто при определяне на път за инвалидни колички и хора с прудности в предвижването;
- Флаг за забрана за използване на върха/реброто при определяне на път за студенти.
